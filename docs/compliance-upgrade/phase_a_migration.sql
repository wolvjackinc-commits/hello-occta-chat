...
-- All new columns are nullable. Existing rows keep NULL values; new sign-ups
-- after Phase C will populate them via server-side validation.
-- =========================================================================
...
-- 2. Snapshot columns on quotes (all nullable, no backfill).
...
-- 4. Snapshot columns on orders (payment method + billing anchor + service snapshot ref).
...
-- 5. Policy version + timestamp columns on contract_acceptances.
-- NOTE: contract_acceptances is now covered by the Phase 0 hard immutability
-- trigger. This ALTER TABLE only ADDS COLUMNS — DDL is not blocked by the trigger.
...
-- SELECT-only grant for customers; writes are edge-service only.
...
-- Immutability trigger for accepted CIP rows (mirrors contract_summaries).
...
-- No customer/authenticated grants: this is admin/service only.
...
-- Immutability: acceptance audit records are append-only.
...
-- 9. Ensure admin_reconciliation_tasks exists (it already does per project inventory,
--    but we ADD an idempotent CREATE guard just in case some environments lack it).
...
GRANT SELECT, INSERT, UPDATE ON public.admin_reconciliation_tasks TO authenticated;
...
DROP POLICY IF EXISTS "Admins manage reconciliation tasks"
  ON public.admin_reconciliation_tasks;
CREATE POLICY "Admins manage reconciliation tasks"
  ON public.admin_reconciliation_tasks FOR ALL TO authenticated
  USING (
    public.has_role(auth.uid(), 'admin'::app_role)
    OR public.has_role(auth.uid(), 'super_admin'::app_role)
  )
...
-- 10. Legacy tagging pass — records only, does not modify accepted rows.
-- (Safe re-run because ON CONFLICT DO NOTHING via a synthetic uniqueness check.)
INSERT INTO public.admin_reconciliation_tasks
  (task_type, priority, related_table, related_record_id, customer_id,
   issue_summary, required_action, status)
SELECT
  ...
  ca.id,
  ca.customer_id,
  'Existing accepted contract predates the two-document (Summary + Information Pack) system. Original record left unchanged.',
  'No action required unless customer requests documentation update. Do not regenerate.',
  'open'
FROM public.contract_acceptances ca
LEFT JOIN public.admin_reconciliation_tasks t
  ON t.related_table = 'contract_acceptances'
 AND t.related_record_id = ca.id
 AND t.task_type = 'legacy_pre_two_document_acceptance'
WHERE t.id IS NULL;
...
INSERT INTO public.compliance_upgrade_runs (
  ...
  jsonb_build_object(
    ...
    'legacy_reconciliation_tasks_created',
      (SELECT count(*) FROM public.admin_reconciliation_tasks
        WHERE task_type = 'legacy_pre_two_document_acceptance')
  )
);