 ...
 *
 * The production session service can return a refreshed token for an older
 * active row before checking that row's expires_at value. Verify resumed
 * sessions before navigating. If the row has expired, rotate the anonymous
 * browser identity once and start a genuinely fresh order instead of sending
 * the customer into an expired-link loop.
 */
