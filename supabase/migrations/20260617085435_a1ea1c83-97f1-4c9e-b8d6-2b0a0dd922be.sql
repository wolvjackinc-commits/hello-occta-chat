-- Phase 4 corrections: atomic outboxes inside confirm_service_live_tx,
-- exact pro-rata maths in Postgres (no JS floats).

-- 1) Add explicit pro-rata bookkeeping columns to first_billing_jobs.
ALTER TABLE public.first_billing_jobs
  ADD COLUMN IF NOT EXISTS period_start date,
  ADD COLUMN IF NOT EXISTS period_end date,
  ADD COLUMN IF NOT EXISTS billable_days int,
  ADD COLUMN IF NOT EXISTS full_cycle_days int,
  ADD COLUMN IF NOT EXISTS calc_method text NOT NULL DEFAULT 'anchor_v1';

-- 2) Replace the transaction function so it ALSO inserts the
--    service-activation outbox row and the first-billing job row in the
--    same atomic transaction. Any failure here rolls back activation.
