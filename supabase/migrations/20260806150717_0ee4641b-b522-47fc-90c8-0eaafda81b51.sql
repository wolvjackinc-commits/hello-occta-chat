-- 1) Harden the caller check inside the routine (defence in depth).
...
BEGIN
  ...
  ELSIF is_backend THEN
    is_super := true; -- trusted backend / service role only
  ELSE
    RETURN jsonb_build_object('success', false, 'error', 'forbidden');
  END IF;
...
-- 2) Remove anonymous / public execute, keep staff + backend only.
...
-- 3) Allow the internal verification role (used by the automated Direct Debit
--    workflow tests) to execute it, when that role exists.
DO $$
BEGIN
