...
BEGIN
  ...
  is_two_doc := jsonb_typeof(cs.one_off_charges_json -> 'snapshot' -> 'components') = 'array';
  IF is_two_doc AND NOT EXISTS (
    SELECT 1 FROM contract_information_packs
    WHERE contract_summary_id = cs.id OR quote_id = q.id
  ) THEN
  ...
  SELECT id, occta_order_number INTO o_id, o_number
    FROM orders
   WHERE checkout_session_id = s.checkout_session_id
      OR journey_id = j.id
   ORDER BY CASE WHEN checkout_session_id = s.checkout_session_id THEN 0 ELSE 1 END
   LIMIT 1
   FOR UPDATE;
  ...
  IF o_id IS NOT NULL THEN
    UPDATE orders
       SET checkout_session_id = coalesce(checkout_session_id, s.checkout_session_id),
           quote_id = coalesce(quote_id, q.id),
           contract_summary_id = coalesce(contract_summary_id, cs.id),
           contract_acceptance_id = coalesce(contract_acceptance_id, acc_id),
           payment_method_id = coalesce(payment_method_id, pm.id),
           guest_order_id = coalesce(guest_order_id, _guest_order_id),
           journey_version = 'v2'
     WHERE id = o_id;
  ELSE
    ...
    IF o_id IS NULL THEN
      SELECT id, occta_order_number INTO o_id, o_number
        FROM orders
       WHERE checkout_session_id = s.checkout_session_id OR journey_id = j.id
       ORDER BY CASE WHEN checkout_session_id = s.checkout_session_id THEN 0 ELSE 1 END
       LIMIT 1;
    ELSE
...
REVOKE ALL ON FUNCTION public.journey2_commit_order(uuid, text, uuid) FROM PUBLIC, anon, authenticated;
